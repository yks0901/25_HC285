...
const float POSITION_EPSILON = 0.09f; // 위치 변화 허용 범위 (예: 3cm)
...
private:
  ...
  std::set<int> published_ids_;  // 이미 퍼블리시된 ID 추적
  ...
  for (const auto& [id, obj_pose_stamped] : object_map_) {
    const auto& obj_pose = obj_pose_stamped.pose;  // ⬅️ 여기서 pose 추출
